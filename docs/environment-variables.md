...
`POSTGRES_USER`는 PostgreSQL을 처음 초기화할 때 `payment` 사용자를 생성하고, `PAYMENT_DB_USERNAME`은 Spring Boot가 그 사용자로 로그인할 때 사용합니다. 두 변수는 자동으로 연결되지 않으며, **최종 사용자·비밀번호·DB 이름 값이 같아서** 접속할 수 있습니다.
