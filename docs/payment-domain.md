...
| React | 주문 화면과 결과 화면을 표시하고 서버 API를 호출하며, 주문 정보를 토스 JS SDK에 전달합니다. 처리 중인 주문의 저장된 결과를 자동 갱신합니다. |
| 토스 JS SDK | React가 넘긴 주문 정보로 토스 결제창 요청을 시작합니다. |
| Spring Boot | 주문과 금액을 검증해 승인을 요청합니다. 미확정 결과를 자동 재조회하고, 같은 거래의 승인 금액·통화가 다르면 취소 후 결과를 저장합니다. |
...
7. **미확정 결과 자동 처리**: 정상 여부를 확정하지 못하면 서버 작업이 재조회합니다. 정상 승인은 성공으로 복구하고, 같은 거래의 승인 금액·통화가 다르면 전액 취소를 요청합니다. 취소 완료를 확인한 뒤 취소 시각을 저장합니다.
8. **주문 내역 표시**: 화면은 저장된 주문과 결제·취소 내역을 읽습니다. 처리 중에는 자동 갱신하며 사용자가 토스 재조회를 직접 요청할 필요가 없습니다.
...
| 조회 식별자 불일치·부분 취소·미지원 수단·재시도 한도 도달 | REVIEW_REQUIRED와 운영자 확인 로그 |
...
주문당 결제 한 건, 비회원, 원화 10,000원, 테스트 키만 사용합니다. 주문 생성·인증·승인·자동 재조회·조건부 취소·결과 조회까지 다룹니다.
