...
이 문서는 현재 구현을 설명합니다. 미확정 결제의 자동 재조회와 잘못된 승인 금액의 자동 취소는 [자동 복구 파이프라인](payment-recovery.md)에 설명합니다.
...
서버 작업이 `PROCESSING`·`UNKNOWN`·`CANCEL_PENDING` 결제를 DB에서 찾아 토스의 `GET /v1/payments/{paymentKey}`로 자동 확인합니다. 사용자는 `GET /orders/{orderId}`로 저장된 주문과 결제·취소 내역을 읽습니다. 수동 재확인 API와 버튼은 없습니다.

정상 승인이라면 성공으로 복구하고, 같은 주문·키의 승인 금액·통화가 다르면 취소 의도와 멱등키를 저장한 후 전액 취소합니다. 취소 응답 유실 시 조회부터 재개합니다. 서버 중단 후에도 작업이 이어지며, 계속 확정하지 못하거나 다른 거래의 응답이면 운영자 확인 상태를 기록합니다. 새 승인을 자동으로 요청하지는 않습니다.
...
로그인·접근 권한, 고객 임의 취소, 가상계좌, 빌링, 웹훅, 배송·정산은 이후 범위입니다. 내부 자동 복구와 금액 불일치 시 전액 취소는 구현되어 있습니다. **결제 승인 완료가 배송 완료나 판매자 계좌의 정산 입금을 뜻하지는 않습니다.**
